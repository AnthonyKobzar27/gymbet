...
serve(async (req)=>{
  ...
  try {
    ...
    // =========================================================================
    if (action === 'list') {
      const { data: withdrawals, error } = await supabase.from('withdrawal_requests').select(`
          id,
          user_id,
          user_hash,
          amount,
    ...
    if (action === 'process') {
      ...
      const { data: withdrawal, error: fetchError } = await supabase.from('withdrawal_requests').select('*').eq('id', withdrawalId).single();
      ...
      try {
        ...
        const { data: recentDeposit, error: depositError } = await supabase.from('webhook_events').select('metadata').eq('user_hash', withdrawal.user_hash).eq('event_type', 'checkout.session.completed').order('processed_at', {
          ascending: false
        }).limit(1).single();
        ...
        const session = await stripe.checkout.sessions.retrieve(recentDeposit.metadata.session_id);
    ...
    if (action === 'process_all') {
      const { data: withdrawals, error } = await supabase.from('withdrawal_requests').select('id').eq('status', 'pending').order('requested_at', {
        ascending: true
      });
